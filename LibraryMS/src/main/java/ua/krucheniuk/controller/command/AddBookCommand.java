...
public class AddBookCommand implements Command {
	...
	public AddBookCommand(LibrarianService librarianService, AuthService authService) {
	...
	public String execute(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			if (yearOfEd.isPresent() & authService.checkYear(yearOfEd.get())) {
				...
				throw new WrongInputException(yearOfEd.get());
			}
			...
			if (quantity.isPresent() & authService.checkQuantity(quantity.get()))
				...
				throw new WrongInputException(quantity.get());

		} catch (WrongInputException ex) {
			log.info("Wrong input: " + ex.getInputData());
